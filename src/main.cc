...
double GetElevation(struct site location) {
    ...
    for (indx = 0, found = 0; indx < MAXPAGES && found == 0;) {
        double xf = ppd * (location.lat - dem[indx].min_north);
        double yf = (double)mpi - (yppd * (LonDiff(dem[indx].max_west, location.lon)));

        if (xf >= 0 && xf < mpi && yf >= 0 && yf < mpi) {
            found = 1;
            ...
            double h00 = dem[indx].data[x0][y0];
            double h10 = dem[indx].data[x1][y0];
            double h01 = dem[indx].data[x0][y1];
            double h11 = dem[indx].data[x1][y1];

            double h0 = h00 * (1.0 - wx) + h10 * wx;
            double h1 = h01 * (1.0 - wx) + h11 * wx;
            double hm = h0 * (1.0 - wy) + h1 * wy;

            elevation = 3.28084 * hm;
        } else {
            indx++;
        }
    }
...
double dist(double lat1, double lon1, double lat2, double lon2)
{
	...
	int delta = equatorRadius-polarRadius;
...
int main(int argc, char *argv[])
{
	...
	if (argc == 1) {
		...
		fprintf(stdout, "          Accepts a directory path to load all terrain files in that folder, e.g.: -lid data/dtm\n");
	...
	if (metric) {
		...
		max_range /= KM_PER_MILE;
		altitude /= METERS_PER_FOOT;
		tx_site[0].alt /= METERS_PER_FOOT;
		tx_site[1].alt /= METERS_PER_FOOT;
	...
	if (ppa == 0) {
	  if (propmodel == 2) {
			...
			if (cropping) {
				...
				cropLon*=dpp;
				max_north=cropLat;
				max_west=cropLon+tx_site[0].lon;
				cropLat-=tx_site[0].lat;
